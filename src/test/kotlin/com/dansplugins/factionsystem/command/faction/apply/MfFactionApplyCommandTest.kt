...
class MfFactionApplyCommandTest {
    ...
    private lateinit var uut: MfFactionApplyCommand
    ...
    fun testCancelApplication() {
        ...
        `when`(player.hasPermission("mf.apply")).thenReturn(true)
        `when`(language["CommandFactionApplyCancelSuccess", targetFactionName]).thenReturn("Application cancelled successfully")

        // execute
        uut.onCommand(player, command, "label", arrayOf(targetFactionName, "cancel"))
    ...
    fun testSendApplication() {
        ...
        // execute
        uut.onCommand(player, command, "label", arrayOf(targetFactionName))
