            ...
            if (sender.hasPermission("mf.create")|| sender.hasPermission("mf.default")) {
                ...
                if (args.length > 1) {
                    ...
                    if (!factionExists) {
                        ...
                        PersistentData.getInstance().getFactions().get(PersistentData.getInstance().getFactions().size() - 1).addMember(player.getUniqueId(), Utilities.getPlayersPowerRecord(player.getUniqueId(), PersistentData.getInstance().getPlayerPowerRecords()).getPowerLevel());
