...
class JooqMfFactionRepository(
    ...
    override fun getFactions(): List<MfFaction> {
        return dsl
            ...
            .map { factionRecord ->
                ...
                val members =
                    dsl
                        .selectFrom(MF_FACTION_MEMBER)
                        .where(MF_FACTION_MEMBER.FACTION_ID.eq(factionRecord.id))
                        .fetch()
                        .map { it.toDomain(roles) }
                val invites =
                    dsl
                        .selectFrom(MF_FACTION_INVITE)
                        .where(MF_FACTION_INVITE.FACTION_ID.eq(factionRecord.id))
                        .fetch()
                        .map { it.toDomain() }
                val applications =
                    dsl
                        .selectFrom(MF_FACTION_APPLICATION)
                        .where(MF_FACTION_APPLICATION.FACTION_ID.eq(factionRecord.id))
                        .fetch()
                        .map { it.toDomain() }
    ...
    private fun upsertMember(
        ...
        dsl
            ...
            .onConflict(MF_FACTION_MEMBER.FACTION_ID, MF_FACTION_MEMBER.PLAYER_ID)
            .doUpdate()
            .set(MF_FACTION_MEMBER.ROLE_ID, member.role.id.value)
            .where(MF_FACTION_MEMBER.FACTION_ID.eq(factionId.value))
    ...
    private fun upsertInvite(
        ...
        dsl
            ...
            .onConflict(MF_FACTION_INVITE.FACTION_ID, MF_FACTION_INVITE.PLAYER_ID)
            .doNothing()
            .execute()
        return dsl
            .selectFrom(MF_FACTION_INVITE)
    ...
    private fun upsertApplication(
        ...
        dsl
            ...
            .onConflict(MF_FACTION_APPLICATION.FACTION_ID, MF_FACTION_APPLICATION.PLAYER_ID)
            .doNothing()
            .execute()
        return dsl
            .selectFrom(MF_FACTION_APPLICATION)
    ...
    override fun delete(factionId: MfFactionId) {
        dsl
            .deleteFrom(MF_FACTION)
            .where(MF_FACTION.ID.eq(factionId.value))
            .execute()
    }

    private fun MfFactionRecord.toDomain(
