...
### Added
- `/faction unclaim auto` (permission `mf.unclaim.auto` or `mf.autounclaim`, default `true`) toggles autounclaim for the sender's faction, the counterpart of `/faction claim auto`. While it is on, a member walking into a chunk their faction has claimed unclaims it, provided their role has the `UNCLAIM` faction permission and they hold `mf.unclaim`. Toggling needs the new `TOGGLE_AUTOUNCLAIM` faction role permission, whose default value is `false`, as with `TOGGLE_AUTOCLAIM`. The setting is stored per faction: a new, additive migration (`V9__Add_autounclaim.sql`) adds an `autounclaim` column defaulting to `false`, and JSON storage writes an `autounclaim` field, read as `false` when absent. Existing factions therefore start with autounclaim off.
