...
### Added
- An opt-in, read-only REST API, served by an embedded Javalin/Jetty server, for other plugins and external tools such as web dashboards, Discord bots and map renderers. `GET /api/factions`, `/api/players`, `/api/relationships` and `/api/claims` (each with by-id lookups, and factions also by name) return JSON built from the plugin's in-memory caches; `/api/openapi` serves an OpenAPI 3.0 description and `/api/health` a liveness check. It is off unless `api.enabled: true` is set in `config.yml`, binds to `127.0.0.1` on port `8080` by default, and has no authentication of its own — access control is the bind address and the host's firewall. Existing servers are unaffected: the code falls back to disabled when the `api` section is absent, so the upgrade boot does not open a socket. See `API_USAGE.md` and `CONFIG.md`. The `slf4j-api` dependency moves from `1.8.0-beta4` to `2.0.7` (the version the build already resolved transitively), and Javalin, Jetty and Jackson are shaded and relocated into the plugin jar.
